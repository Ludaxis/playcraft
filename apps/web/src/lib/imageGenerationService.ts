...
export function getPromptSuggestions(category: AssetCategory): string[] {
  const suggestions: Record<AssetCategory, string[]> = {
    ...
    ui: [
      'Flat rounded rectangle button, blue gradient, for play button',
      'Square panel frame with golden border for inventory slot',
      'Red heart icon for health indicator',
      'Flat coin icon with gold color for score display',
      'Simple pause icon, two vertical bars, white on dark',
    ],
    tile: [
      'Blue numbered tile for 2048 puzzle game, number 2',
      'Green grass texture tile, top-down view',
      'Stone floor tile, seamless pattern',
      'Wooden plank tile for puzzle board',
      'Colorful candy tile for match-3 game',
    ],
    effect: [
