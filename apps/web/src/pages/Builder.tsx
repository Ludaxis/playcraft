...
export function BuilderPage({
  ...
  // Ref to track latest project name for callbacks (avoids stale closure)
  const projectNameRef = useRef(initialProject.name);
  ...
  const { messages, isGenerating, generationProgress, sendMessage: sendAiMessage, cancelGeneration, addSystemMessage, suggestions } =
    usePlayCraftChat({
      projectId: project.id,
      ...
      onGameNameDetected: async (detectedName) => {
        // Use ref to get latest project name (avoids stale closure)
        const currentName = projectNameRef.current;
        // Always update if current name is placeholder or looks auto-generated
        const isPlaceholder = currentName === 'Untitled Game' || currentName === 'New Game';
        const looksAutoGenerated = /^(I want|Make|Create|Build|A |An |The )/i.test(currentName);

        if (isPlaceholder || looksAutoGenerated) {
          console.log('[Builder] Updating project name from code title:', detectedName, '(was:', currentName, ')');
          try {
            await updateProject(project.id, { name: detectedName });
            setProject(prev => ({ ...prev, name: detectedName }));
            projectNameRef.current = detectedName; // Update ref immediately
          } catch (err) {
            console.warn('[Builder] Failed to update project name:', err);
          }
        } else {
          console.log('[Builder] Keeping existing project name:', currentName, '(detected:', detectedName, ')');
        }
      },
