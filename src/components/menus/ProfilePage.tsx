...
export function ProfilePage() {
  ...
  return (
    <div className="flex flex-col h-full bg-secondary">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        <div className="bg-secondary-light rounded-xl border-2 border-surface-dark p-4">
          <div className="flex items-center gap-4">
            ...
            <button
              ...
              className="relative"
            >
              <div className="w-24 h-24 bg-surface-dark rounded-xl border-2 border-surface flex items-center justify-center overflow-hidden">
                <Image
              ...
              {/* Edit indicator */}
              <div className="absolute -bottom-1 -right-1 w-7 h-7 bg-surface rounded-lg border-2 border-surface-light flex items-center justify-center">
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-surface text-sm">05/2022</span>
              </div>
            </div>

            {/* Level Badge */}
            <div className="bg-surface-dark rounded-lg px-3 py-2 border-2 border-surface">
              <div className="text-secondary text-xs font-bold text-center">Level</div>
              <div className="text-primary-light text-xl font-bold text-center">{player.currentLevel}</div>
            </div>
          </div>
        </div>
        ...
        <div className="relative">
          ...
          <div className="flex justify-center mb-2">
            <div className="bg-surface-dark rounded-lg px-6 py-1 border-2 border-surface relative">
              {/* Ribbon ends */}
              <div className="absolute -left-2 top-1/2 -translate-y-1/2 w-0 h-0 border-t-[10px] border-b-[10px] border-r-[8px] border-t-transparent border-b-transparent border-r-secondary-light" />
              <div className="absolute -right-2 top-1/2 -translate-y-1/2 w-0 h-0 border-t-[10px] border-b-[10px] border-l-[8px] border-t-transparent border-b-transparent border-l-secondary-light" />
      ...
      <div className="bg-primary-light border-t-2 border-secondary opacity-50">
...
function StatItem({ icon, label, value }: StatItemProps) {
  return (
    <div className="flex flex-col items-center">
      ...
      {/* Label */}
      <p className="text-secondary text-[9px] font-medium text-center whitespace-nowrap mb-1">
        {label}
      </p>
      {/* Value */}
      <div className="bg-surface rounded px-3 py-0.5 w-full">
        <p className="text-secondary text-sm font-bold text-center">{value}</p>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="flex flex-col items-center px-3 py-1">
      <div className="w-10 h-10 bg-secondary rounded-lg flex items-center justify-center">
        <span className="text-surface-dark text-xs font-bold">{icon}</span>
      </div>
      {label && <span className="text-surface-dark text-[10px] mt-0.5">{label}</span>}
    </div>
  );
}
