...
export function ExportButton({ targetId = 'app-content' }: ExportButtonProps) {
  ...
  return (
    <>
      {/* Notification Toast */}
      {notification && (
        <div className="fixed top-4 left-1/2 -translate-x-1/2 z-[200] bg-bg-inverse text-text-inverse px-4 py-2 rounded-lg border border-border text-caption">
          {notification}
        </div>
      )}
      ...
      {isSelectMode && (
        <div className="fixed inset-0 z-[150] pointer-events-none">
          <div className="absolute top-4 left-1/2 -translate-x-1/2 bg-bg-inverse/80 text-text-inverse px-4 py-2 rounded-full text-caption">
            Click on any component to select it (ESC to cancel)
          </div>

          {hoveredElement && (
            <div
              className="absolute border-2 border-brand-primary bg-brand-primary/20 rounded transition-all duration-75"
              style={{
      ...
      {selectedElement && (
        <div className="export-dialog fixed inset-0 z-[160] flex items-center justify-center bg-black/50" onClick={cancelAll}>
          <div className="bg-bg-card rounded-xl border-2 border-border p-4 min-w-[300px]" onClick={(e) => e.stopPropagation()}>
            <h3 className="font-bold text-text-primary mb-3">Export Component</h3>

            <div className="bg-bg-muted rounded-lg p-3 mb-4">
              <div className="text-mini text-text-muted mb-1">Selected:</div>
              <div className="text-caption text-text-secondary truncate font-mono">
                {selectedElement.tagName.toLowerCase()}
                {selectedElement.className && (
                  <span className="text-text-muted">.{selectedElement.className.split(' ')[0]}</span>
                )}
              </div>
              <div className="text-mini text-text-muted mt-1">
            ...
            <div className="space-y-2">
              <button
                ...
                className="w-full px-4 py-3 bg-brand-primary hover:bg-brand-hover text-text-inverse rounded-lg font-medium flex items-center justify-center gap-2 disabled:opacity-50"
              >
              ...
              <button
                ...
                disabled={isExporting}
                className="w-full px-4 py-2.5 bg-brand-hover hover:bg-brand-primary text-text-inverse rounded-lg font-medium flex items-center justify-center gap-2 disabled:opacity-50"
              >
              ...
              <button
                ...
                disabled={isExporting}
                className="w-full px-4 py-2.5 bg-bg-muted hover:bg-border text-text-secondary rounded-lg font-medium flex items-center justify-center gap-2 disabled:opacity-50"
              >
                <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
              ...
              <button
                type="button"
                onClick={cancelAll}
                className="w-full px-4 py-2 text-text-muted hover:text-text-secondary font-medium"
              >
                Cancel
              </button>
            </div>
      ...
      <div className="export-button-container fixed bottom-20 right-3 z-[100]">
        {isOpen && (
          <div className="absolute bottom-14 right-0 bg-bg-card rounded-xl border-2 border-border overflow-hidden min-w-[220px]">
            <div className="px-3 py-2 bg-bg-muted border-b border-border">
              <span className="text-mini font-semibold text-text-muted uppercase">Full Page</span>
            </div>
            ...
            <button
              ...
              disabled={isExporting}
              className="w-full px-4 py-3 text-left text-caption font-medium text-text-secondary hover:bg-brand-muted flex items-center gap-3 disabled:opacity-50"
            >
              ...
              <div>
                ...
                <div className="text-mini text-brand-primary">Best for Figma</div>
              </div>
            </button>
            ...
            <button
              ...
              disabled={isExporting}
              className="w-full px-4 py-3 text-left text-caption font-medium text-text-secondary hover:bg-bg-muted flex items-center gap-3 disabled:opacity-50"
            >
              ...
              <div>
                ...
                <div className="text-mini text-text-muted">High resolution image</div>
              </div>
            </button>
            ...
            <button
              ...
              disabled={isExporting}
              className="w-full px-4 py-3 text-left text-caption font-medium text-text-secondary hover:bg-bg-muted flex items-center gap-3 disabled:opacity-50 border-b border-border"
            >
              <svg className="w-5 h-5 text-brand-primary" viewBox="0 0 24 24" fill="currentColor">
                <path d="M12 2L2 7v10l10 5 10-5V7L12 2zm0 2.8L18.6 7 12 9.2 5.4 7 12 4.8z"/>
              </svg>
              <div>
                <div>SVG</div>
                <div className="text-mini text-text-muted">Limited Figma support</div>
              </div>
            </button>

            <div className="px-3 py-2 bg-bg-muted border-b border-border">
              <span className="text-mini font-semibold text-text-muted uppercase">Component</span>
            </div>
            <button
              type="button"
              onClick={startComponentSelection}
              className="w-full px-4 py-3 text-left text-caption font-medium text-text-secondary hover:bg-bg-muted flex items-center gap-3"
            >
              <svg className="w-5 h-5 text-brand-primary" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              ...
              <div>
                ...
                <div className="text-mini text-text-muted">Export individual element</div>
              </div>
            </button>
          </div>
        )}
        ...
        >
          {isExporting ? (
            <svg className="w-5 h-5 text-text-inverse animate-spin" viewBox="0 0 24 24" fill="none">
              <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/>
              <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"/>
            </svg>
          ) : isSelectMode ? (
            <svg className="w-6 h-6 text-text-inverse" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
