    ...
    >
      <div className="p-4">
        ...
        <Panel variant="elevated" className="mb-4 text-center">
          ...
          <p className="text-4xl font-bold text-slate-800 mb-2">{timer.formatted}</p>
          <p className="text-xs text-slate-500">Complete levels to earn bonus rewards!</p>
        </Panel>

        {/* Progress */}
        <Panel variant="elevated" className="mb-4">
          <div className="flex justify-between items-center mb-2">
            <span className="text-sm text-slate-600">Levels Completed</span>
        ...
        {/* Rewards */}
        <h3 className="text-sm font-semibold text-slate-800 mb-3">Rush Rewards</h3>
          ...
          {Array.from({ length: event.maxProgress }).map((_, i) => (
            <div
              ...
              className={`
                ...
                ${i < event.progress ? 'bg-slate-800 text-white' : 'bg-slate-200 text-slate-600'}
              `}
            >
              {i + 1}
            </div>
        ...
        <Panel variant="outlined" className="text-center">
          <p className="text-sm text-slate-600 mb-2">Complete All Levels</p>
          <div className="w-16 h-16 bg-slate-200 rounded-lg mx-auto mb-2 flex items-center justify-center">
            <span className="text-slate-400 text-xs">[Prize]</span>
          </div>
          <p className="font-bold text-slate-800">3 Extra Lives</p>
          {event.progress === event.maxProgress ? (
